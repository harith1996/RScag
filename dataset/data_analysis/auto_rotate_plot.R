...
rootDirectory <- "D:/InfoVis/ROTATE/"
issueDirectory <- "D:/InfoVis/ROTATE/"
outDirectory <- "D:/InfoVis/ROTATE/output/"
suffix <- ".csv"
issueName <- "/issue.csv"
...
auto_rotate_plot <- function(filename, angle, head, outPngName, outCSVName, outDataName)
{
  ...
  y1 <- (y - y0)*cos(angle) + (x - x0)*sin(angle) + y0
  ...
  ggsave(plot = p, filename = outPngName, width = 1.5, height = 1.5)
